...
def main():
    if (args.install):
        print(
            "Press any key to continue, pip install functions will be executed."
        )
        ...
        subprocess.call(
            'pip install --upgrade python-socketio[client]', shell=True)
        subprocess.call('pip install --upgrade imutils', shell=True)
        subprocess.call('pip install --upgrade requests', shell=True)
        subprocess.call('pip install --upgrade pillow', shell=True)
    elif (args.cov):
        subprocess.call(
            'coverage run -m unittest discover . "*Test.py"', shell=True)
